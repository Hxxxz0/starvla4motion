...
def _use_deepspeed() -> bool:
    return os.environ.get("STARVLA_USE_DEEPSPEED", "0").lower() in {"1", "true", "yes", "on"}
...
class VLATrainer(TrainerUtils):
    ...
    def eval_action_model(self, step_metrics: dict = None) -> float:
        ...
        predict_kwargs = {"examples": examples, "use_ddim": True, "num_ddim_steps": 20}
        if hasattr(self.model, "action_horizon"):
            target_horizon = np.array(actions).shape[1]
            if target_horizon == getattr(self.model, "action_horizon", None):
                predict_kwargs["num_chunks"] = 1

        output_dict = self.model.predict_action(**predict_kwargs)
        ...
        if self.accelerator.is_main_process:
            ...
            if normalized_actions.shape[1] != actions.shape[1]:
                if normalized_actions.shape[1] > actions.shape[1]:
                    ...
                    raise ValueError(
                        f"Predicted horizon {normalized_actions.shape[1]} is shorter than ground truth "
                        f"horizon {actions.shape[1]} during evaluation."
                    )
...
if __name__ == "__main__":
    ...
    if cfg.is_debug and _dist_ready() and dist.get_rank() == 0:
        import debugpy
